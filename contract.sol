...
contract MetaBull is Context, IERC20, Ownable {
    ...
    address payable public buybackAddress; // BuyBack Address
        
    address payable public teamAddress; // BuyBack Address
        
    address payable public developmentAddress; // BuyBack Address
    ...
    uint256 public _maxWalletToken = _tTotal.div(100) * (10 ** _decimals); 
    uint256 public maxTxnAmount = _tTotal.div(100) * (10 ** _decimals); 

    string private constant _name = "ANIMALS";
    string private constant _symbol = "ANIM";
    ...
    uint256 private gasPriceLimit = 50  gwei; 
    uint256 private gasMaxLimit = 1000000 gwei; // gasLeft limit
    
    // store addresses that a automatic market maker pairs. Any transfer *to* these addresses
    // could be subject to a maximum transfer amount
    mapping (address => bool) public automatedMarketMakerPairs;

    uint256 private minimumTokensBeforeSwap = _tTotal.mul(5).div(10000); // 0.05%
    ...
    function _transfer(
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            if (automatedMarketMakerPairs[from]) {
                removeAllFee();
                _taxFee = _buyTaxFee;
                _liquidityFee = _buyLiquidityFee + _buyMarketingFee + _buyBuybackFee;
            } 
            // Sell
                ...
                // 6x tax if bought in the same block as trading active for 72 hours
                if(boughtEarly[from]){
                    _taxFee = _taxFee * 6;
                    _liquidityFee = _liquidityFee * 6;
                }
    ...
    function swapBack() private lockTheSwap {
        ...
        uint256 bnbForOperations = bnbBalance.mul(_marketingTokensToSwap).div(totalTokensToSwap);
        
        uint256 bnbForBuyBack = bnbBalance.mul(_buyBackTokensToSwap).div(totalTokensToSwap);
        uint256 bnbForMarketing = bnbForOperations * 20 / 70;
        uint256 bnbForTeam = bnbForOperations * 25 / 70;
        uint256 bnbForDevelopment = bnbForOperations * 25 / 70;
        
        uint256 bnbForLiquidity = bnbBalance.sub(bnbForMarketing).sub(bnbForBuyBack).sub(bnbForTeam).sub(bnbForDevelopment);
        ...
        (bool success,) = address(buybackAddress).call{value: bnbForBuyBack}("");
        (success,) = address(marketingAddress).call{value: bnbForMarketing}("");
        (success,) = address(developmentAddress).call{value: bnbForDevelopment}("");
        (success,) = address(teamAddress).call{value: bnbForTeam}("");
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        if(_liquidityFee ==  _buyLiquidityFee + _buyMarketingFee + _buyBuybackFee){
        ...
        } else if(_liquidityFee == _sellLiquidityFee + _sellMarketingFee + _sellBuybackFee){
    ...
    function setBuyFee(uint256 buyTaxFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyBuybackFee)
        external
        onlyOwner
    {
        ...
        require(_buyTaxFee + _buyLiquidityFee + _buyMarketingFee + _buyBuybackFee <= 20, "Must keep taxes below 20%");
    }
        ...
        onlyOwner
    {
        ...
        require(_sellTaxFee + _sellLiquidityFee + _sellMarketingFee + _sellBuybackFee <= 30, "Must keep taxes below 30%");
    }
